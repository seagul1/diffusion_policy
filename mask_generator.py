...
class LowdimMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        ...
        # generate obs mask
        # Batch中每一项的 n_obs_steps？
        # 一维长度为B，固定obs_steps就都取max_n_obs_steps，否则obs_steps则每一个元素随机从[1, max_n_obs_steps]选
        if self.fix_obs_steps:
            # obs_steps = tensor([max_n_obs_steps, ...]) (length = B)
        ...
        # steps = tensor([[0, ... , T-1], ... [0, ... , T-1]])
        # 创建一个形状为(B, T)的张量，其中每个元素的值为其在序列中的位置。
        steps = torch.arange(0, T, device=device).reshape(1,T).expand(B,T)
        # 如果该时间步的索引小于obs_steps张量中对应的值，每个时间步的观测维度都被设置为True
        obs_mask = (steps.T < obs_steps).T.reshape(B,T,1).expand(B,T,D)
        # 通过与操作只将对应的obs维度设置为True，去掉上步一起将action和obs一起设置为True中的action部分
        obs_mask = obs_mask & is_obs_dim

        # generate action mask
        if self.action_visible:
            # TODO：这里obs_steps - 1是为什么？最后一步是obs，没有动作，所以是obs_steps - 1？
