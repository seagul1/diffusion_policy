...
class RTDEInterpolationController(mp.Process):
    ...
    def run(self):
        ...
        """
        os.sched_setscheduler: 这是调用操作系统的函数来设置进程的调度策略和参数的功能。在Python中，os 模块提供了与操作系统交互的功能。
        0: 这是要设置调度策略的进程的进程ID。0 表示当前进程，因此这个调用将应用于当前进程。
        os.SCHED_RR: 这是调度策略的标志，表示使用轮转调度（Round-Robin Scheduling）。轮转调度是一种常见的实时调度策略，它为每个进程分配一个时间片，在时间片用尽之后，进程被放回队列尾部，并允许下一个进程执行。
        os.sched_param(20): 这是设置调度参数的对象，其中的数字20表示进程的优先级。这个数字越高，优先级就越高，系统会更频繁地选择这个进程来执行。
        """
        ...
        try:
            ...
            # time.monotonic() 方法返回的时间值是单调递增的，与系统时间无关。它主要用于测量时间间隔，
            # 特别是在需要测量经过的时间而不受系统时间更改的影响时。
            ...
            while keep_running:
                ...
                # This function is used in combination with waitPeriod() and
                # is used to get the start of a control period / cycle.
